...
## TODO : IL YA PLUS DE FEATURES A MODIFIER QUE CELLE CI ???? M \in 1:6
...
monitoring_main <- function(data,
  ...
  for(TARGET_start in seq.Date(from=as.Date(start, origin="1970-01-01"), to=as.Date(end, origin="1970-01-01"), by=delay_update)){
    ...
    agg_period_drifted[, price_related_col] = abs(rnorm(nrow(test), .5, 1))*test_drifted[, ..price_related_col]
    
    # Predict
    y_pred <- agg_period_raw$VAR_REP
    X_preds <- c(agg_period_raw[, -c("Customer.ID", "VAR_REP", "YEAR")], 
    ...
    for(idx in 1:2){
      ...
      res_drift <- drift_score(X_train, X_pred)
    ...
    if(kind_target == "factor"){
      ...
      for(size in c(100,200)){
        out_period[paste0("TauxAchat-TOP-",size)] = c(mean(y_pred[order(pred_test,decreasing = TRUE)[1:size]]),
                                                      mean(y_pred[order(pred_drifted,decreasing = TRUE)[1:size]]))
      }
      
      for(size in c(5700,5800)){
        out_period[paste0("TauxAchat-DOWN-",size)] = c(mean(y_pred[order(pred_test)[1:size]]),
                                                       mean(y_pred[order(pred_drifted)[1:size]]))
      }
    }
...
scores = monitoring_main(data, train_6M[, -c("Customer.ID", "VAR_REP", "YEAR")], "2011-01-01", "2011-12-31", xgb$model)
